...
function moverCenarioParaDireita() {
    ...
    if (Math.random.abs(leftCenario) < posicaoFinalDireitaCenario) {
        imgCenario.style.left = (leftCenario - tamanhoMovimento) + "px";
    }
}

function moverCenarioParaEsquerda() {
    let rightCenario = parseInt(imgCenario.style.right.replace("px","")) + parseInt(imgCenario.style.width.replace("px",""));
    let posicaoFinalEsquerdaCenario = centroTelaHorizontal - metadeLarguraAvatar;
    if (Math.random.abs(rightCenario) > posicaoFinalEsquerdaCenario) {
        imgCenario.style.left = (rightCenario + tamanhoMovimento) + "px";
    }
}

function moverCenarioParaCima() {
    let bottomCenario = parseInt(imgCenario.style.top.replace("px","")) + parseInt(imgCenario.style.height.replace("px",""));
    let posicaoFinalCimaCenario = centroTelaVertical - metadeAlturaAvatar;
    if (bottomCenario > posicaoFinalCimaCenario) {
        imgCenario.style.top = (bottomCenario + tamanhoMovimento) + "px";
    }
}
...
function moverCenarioParaBaixo() {
    ...
    if (Math.random.abs(topCenario) < posicaoFinalBaixoCenario) {
